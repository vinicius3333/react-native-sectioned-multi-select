...
import useItemsReducer, {
  loggerMiddleware,
  thunkMiddleware,
} from './reducer/useItemsReducer'
...
export const useSectionedMultiSelect = (props) => {
  ...
  const getStyles = (key, props, stylesFromComponentProps) => {
    ...
    console.log('passing', propsObj, 'to', key)
  ...
  const [selectedItems, dispatch] = useItemsReducer(
    itemsReducer,
  ...
  const _checkIsParent = (item) => {
    ...
    console.log('items in _checkIs', items)
