...
class RowItem extends Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    if (nextProps.selectedItems !== this.props.selectedItems) {
      if (
        ...
        !nextProps.selectedItems.includes(this.props.getItem(this.props.item))
      ) {
        return true
      }
      if (
      ...
      if (
        ...
        this.props.item[this.props.subKey].findIndex(el =>
          nextProps.selectedItems.includes(this.props.getItem(el))
        ) !== -1
      ) {
        return true
      }
      if (
        this.props.item[this.props.subKey] &&
        this.props.item[this.props.subKey].findIndex(el =>
          this.props.selectedItems.includes(this.props.getItem(el))
        ) !== -1
      ) {
        return true
      }
  ...
  _itemSelected = item => {
    const { getItem, selectedItems } = this.props
    return selectedItems.includes(getItem(item))
  }
  ...
  _toggleSubItem = item => {
    ...
    if (subToggled === getItem(item)) {
      this.setState({ subToggled: false })
    } else {
      this.setState({ subToggled: getItem(item) })
    }
  ...
  _hasChildren = () => {
    const { getChildren, item } = this.props
  ...
  render() {
    ...
    const hasDropDown = this._hasChildren() && showDropDowns
