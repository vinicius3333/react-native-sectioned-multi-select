...
const Search = ({searchAdornment: searchAdornmentFromProps, styles: stylesFromProps = {}}) => {
  const {
    hideSearch,
    components,
  ...
  const searchBarStyles =  React.useMemo( 
    ...
    [stylesFromProps.searchBar, styles.searchBar, searchTerm, renderItems.length])
    const searchTextInputStyles =  React.useMemo( 
      () => getStyles('searchTextInput', searchStyleParams, stylesFromProps),
      [stylesFromProps.searchTextInput, styles.searchTextInput, searchTerm, renderItems.length])
