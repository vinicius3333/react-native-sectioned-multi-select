...
export const Chips = ({ children }) => {
  ...
  const { components, styles, colors, single, _removeAllItems, selectedItems, showRemoveAll, removeAllText, showChips, } = useSMSContext();
  ...
  return children ? (children) : selectedItems.length > 0 &&
    ...
    showChips ? (<View style={[
      ...
      {selectedItems && selectedItems.length > 0 && selectedItems.map(id => {
        return <components.Chip key={id} id={id} selectedItems={selectedItems} />;
      })}
    </View>) : null;
};
...
const ChipWrapper = (Comp) => (props) => {
  const { components, items, styles: stylesFromContext, colors, displayKey, Icon, hideChipRemove, hideParentChips, iconNames, selectedItems, _toggleChildren, _toggleItem, getChildren, itemId, childItemId, _checkIsParent, } = useSMSContext();
  ...
  if (!item || !item[displayKey] || hideChip)
    return null;
  return (<Comp onPress={onPress} styles={styles} colors={colors} displayKey={displayKey} Icon={Icon} isParent={isParent} ChipRemoveIcon={components.ChipRemoveIcon} hideChipRemove={hideChipRemove} hideParentChips={hideParentChips} iconNames={iconNames} item={item} selectedItems={selectedItems} />);
};

export const Chip = ChipWrapper(React.memo((props) => {
  const { ChipRemoveIcon, styles, colors, displayKey, item, onPress, Icon, hideChipRemove, iconNames, isParent, } = props;
