...
export const ItemSeparator = ({ children, styles: stylesFromProps = {}, ...rest }) => {
  const {
    styles,
    getStyles
  } = useSMSContext()
  const separatorStyles = React.useMemo(() => getStyles('separator', {}, stylesFromProps), [styles.separator, stylesFromProps.separator])
...
export const SubItemSeparator = ({ children, styles: stylesFromProps = {}, ...rest }) => {
  const {
    styles,
    getStyles
  } = useSMSContext()
  const subSeparatorStyles = React.useMemo(() => getStyles('subSeparator', {}, stylesFromProps), [styles.subSeparator, stylesFromProps.subSeparator])

  return React.useMemo(() => {
    console.log('SubItemSep render')
    return (
      <View
        style={subSeparatorStyles}
        {...rest}
      >
        {children && children}
      </View>
...
const Items = ({ flatListProps: flatListPropsFromComponent, styles: stylesFromProps = {} }) => {
  const {
    components,
    items,
    selectedItems,
  ...
  const itemWrapperStyles = React.useMemo(() => getStyles('itemWrapper', {}, stylesFromProps), [styles.itemWrapper, stylesFromProps.itemWrapper])
  return (
    <View
