...
const compose = (...fns) => (value) => fns.reduceRight((prevValue, fn) => fn(prevValue), value);



export const thunkMiddleware = state => dispatch => action => {
...
export const loggerMiddleware = (state, reducer) => dispatch => action => {
...
export const useItemsReducer = (    reducer,    initalState,    middlewares = [],  ) => {
...
export const useSectionedMultiSelect = props => {
  ...
  const [selectedItems, dispatch] = useItemsReducer(itemsReducer, props.initialSelectedItems, [thunkMiddleware, props.debug && loggerMiddleware])
  ...
  const onSelectedItemsChangeInternal = (dispatchParams) => {
    ...
    !itemsControlled && dispatch(dispatchParams) // we need this to be synchronous so selectedItems is definitely updated when we pass them to onSelectedItemsChange?
