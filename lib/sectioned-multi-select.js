...
export const useSectionedMultiSelect = props => {
  let Icon = View // require('react-native-vector-icons/MaterialIcons').default
  ...
  const loadIcon = React.useCallback(() => {
    if (isMount.current) return
    console.log('load icon effect top')

    if (props.iconRenderer) {
      console.log('load icon renderer')
      Icon = props.iconRenderer
      iconLoaded = true
    } else {
      Icon = require('react-native-vector-icons/MaterialIcons').default
      iconLoaded = true
      console.log('icon loaded,', Icon)
    }
  })
  ...
  const find = (id, items, isSub) => {
    if (!items) {
      return {}
    }
    const { getChildren, getItem, getChildItem } = getProps()
    const getFn = isSub ? getChildItem : getItem
    let i = 0
    let found
  ...
  const _removeItem = item => {
    ...
    highlightChildren && _unHighlightChildren(itemToRemove)
  ...
  const _toggleItem = item => {
    ...
    const itemToToggle = isParent ? getItem(item) : getChildItem(item)
    ...
    for (; i < items.length; i += 1) {
      if (getItem(items[i]) === id && Array.isArray(getChildren(items[i]))) {
        getChildren(items[i]).forEach(sub => {
          !highlighted.includes(getItem(sub)) && highlighted.push(getItem(sub))
        })
      }
    }
  ...
  const _displaySelectedItems = (
    ...
    const { styles, colors } = state
    return selectedItemsInternal.map(singleSelectedItem => {
      const item = _findItem(singleSelectedItem)
      ...
      return !hideChip ? (
        <View
          ...
          key={isParent ? getItem(item) : getChildItem(item)}
        >
          <Text
            numberOfLines={2}
            style={[
