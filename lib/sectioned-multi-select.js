...
export const useSectionedMultiSelect = props => {
  ...
  const _checkIsParent = item => {
    ...
    console.group('checkis', item)
    // this should return true if item has children or if it's
    // a top-level item with no children
    if (getChildren(item)) {
      console.log('has children, returning')
      console.groupEnd()
      return true;
    }
  ...
  const _toggleItem = item => {
    ...
    console.log('checkisparent in _toggleItem')
