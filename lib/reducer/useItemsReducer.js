...
// from https://github.com/coinbase/rest-hooks/tree/master/packages/use-enhanced-reducer

function compose(fns) { return (initial) => fns.reduceRight((prev, fn) => fn(prev), initial) };

export default function useItemsReducer (reducer, initalState, middlewares = []) {
...
export function loggerMiddleware({ getState, dispatch, }) { 
  return (next) => async (action) => {
    ...
    console.info('%cPrevious State:', 'color: salmon; font-weight: 700;', getState());
    await next(action);
